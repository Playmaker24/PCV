...
{
    float radiants_phi = phi*M_PI/180;
    return cv::Matx33f(cos(radiants_phi), -sin(radiants_phi), 0, 
                        sin(radiants_phi), cos(radiants_phi), 0, 
                        0, 0, 1);
}
...
{
    float dot_product = point.dot(line);
    return  (dot_product <= eps) && (dot_product >= -eps);
}
