...
std::vector<cv::Vec4f> applyH_3D_points(const std::vector<cv::Vec4f>& geomObjects, const cv::Matx44f &H)
{
    ...
    for (const auto& point : geomObjects) {
...
cv::Matx44f computeCameraPose(const cv::Matx33f &K, const std::vector<cv::Vec3f>& p1, const std::vector<cv::Vec3f>& p2)
{
    ...
    Matx33f R1 = U * W * Vt;
    Matx33f R2 = U * W.t() * Vt;        
    cv::Vec3f t(U(0, 2), U(1, 2), U(2, 2));

    Matx34f cameraPoses[4] = {
        Matx34f(R1(0, 0), R1(0, 1), R1(0, 2), t(0),
        ...
        Matx34f(R2(0, 0), R2(0, 1), R2(0, 2), t(0),
    ...
    Matx34f P1 = Matx34f::eye();
    int bestCount = -1;
    cv::Matx44f bestPose;
    for (const auto &P2 : cameraPoses) {
        int count = 0;
        std::vector<cv::Vec4f> result = linearTriangulation(P1, P2, p1, p2);
...
void BundleAdjustment::BAState::computeResiduals(float *residuals) const
{
    ...
    for (unsigned camIdx = 0; camIdx < m_cameras.size(); camIdx++) {
        ...
        cv::Matx44f H = cameraState.H;                          
        cv::Matx34f P = K * H.get_minor<3, 4>(0, 0);   
...
void BundleAdjustment::BAState::computeJacobiMatrix(JacobiMatrix *dst) const
{
    ...
    for (unsigned camIdx = 0; camIdx < m_cameras.size(); camIdx++) {
        ...
        for (const KeyPoint &kp : m_scene.cameras[camIdx].keypoints) {
            ...
            // cv::Vec4f X = cv::Vec4f(trackState.location(0) / trackState.location(3), 
            //                         trackState.location(1) / trackState.location(3), 
            //                         trackState.location(2) / trackState.location(3)
            //                         1.0f);
            cv::Vec4f X = trackState.location;
            ...
            du_dDeltaK(0, 0) = v(0) * K(0, 0);
            du_dDeltaK(0, 1) = v(2) * K(0, 2);
            du_dDeltaK(0, 2) = 0;
            du_dDeltaK(1, 0) = v(1) * K(1, 1);
            du_dDeltaK(1, 1) = 0;
            du_dDeltaK(1, 2) = v(2) * K(1, 2);
            du_dDeltaK(2, 0) = 0;
            du_dDeltaK(2, 1) = 0;
            du_dDeltaK(2, 2) = 0;
            ...
            J.m_rows[rIdx].J_internalCalib = kp.weight * J_hom2eucl * du_dDeltaK;
            ...
            // Remember to include the weight of the keypoint (kp.weight)
            J.m_rows[rIdx].J_camera = kp.weight * J_hom2eucl * K * dv_dDeltaH;
            ...
            cv::Matx<float, 2, 4> J_worldSpace2eucl = J_hom2eucl * K * H.get_minor<3, 4>(0, 0);
            ...
            J.m_rows[rIdx].J_track = kp.weight * J_worldSpace2eucl;
...
void BundleAdjustment::BAState::update(const float *update, State *dst) const
{
    ...
    for (unsigned i = 0; i < m_cameras.size(); i++) {
        ...
        */
        ...
        cv::Matx44f rotationX = rotationMatrixX(update[cameraOffset + i * NumUpdateParams::CAMERA + 0]);
        cv::Matx44f rotationY = rotationMatrixY(update[cameraOffset + i * NumUpdateParams::CAMERA + 1]);
        cv::Matx44f rotationZ = rotationMatrixZ(update[cameraOffset + i * NumUpdateParams::CAMERA + 2]);
        cv::Matx44f translation = translationMatrix(
